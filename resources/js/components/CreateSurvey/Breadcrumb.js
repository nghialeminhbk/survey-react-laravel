...
const BreadcrumbCreate= ({surName, id}) => {
    return ( 
        <section className="workspace d-flex" >
            ...
            <div className="d-flex align-items-center px-3 border-start border-2">
                <Link to={"/surveys/" + id} disabled className="px-1 text-decoration-none text-muted">Result</Link>
                <Link to={"/surveys/"+id+"/create"} className="px-1 border-bottom border-dark border-2 text-decoration-none text-dark fw-bold">Create</Link>
            </div>
        </section>
     );
}
